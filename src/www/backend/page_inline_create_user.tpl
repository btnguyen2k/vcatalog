...
<body class="[:$DOJO_THEME:]">
   ...
   <div id="[:$FORM.name|escape:'html':]" class="align-center viewport-800" name="[:$FORM.name|escape:'html':]" dojoType="dijit.form.Form" 
        ...
        <br />  
      	...
      	<label for="form_password">[:$LANG->getMessage('msg.user.password'):]:</label>
		<input type="password" id="form_password" style="width: 50%"
			dojoType="dijit.form.ValidationTextBox" regExp="([\w]{5})(\w)*(\d)*"			
			invalidMessage="[:$LANG->getMessage('error.passwordMoreThan6characters'):]"
			name="password" intermediateChanges=false
			required="true" missingMessage="[:$LANG->getMessage('msg.newPasswordIsRequired'):]"/>
		<br/>
		
		<label for="form_confirmedPassword">[:$LANG->getMessage('msg.user.confirmedPassword'):]:</label>
		<input type="password" name="confirmPassword" style="width: 50%"
			id="form_confirmedPassword" dojoType="dijit.form.ValidationTextBox"
			required="true" constraints="{'other': 'form_password'}"
